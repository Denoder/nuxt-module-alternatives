...
export default defineNuxtRouteMiddleware(async (to, from) => {
    ...
    if (Object.prototype.hasOwnProperty.call(to.meta, 'auth') && routeMeta('auth', false)) {
        return;
    }
    ...
    const pageIsInGuestMode = Object.prototype.hasOwnProperty.call(to.meta, 'auth') && routeMeta('auth', 'guest');
    ...
    if (ctx.$auth.$state.loggedIn) {
        ...
        if (!login || insidePage(login) || pageIsInGuestMode) {
            ctx.$auth.redirect('home', true, to);
        }
    ...
    // with `auth: false` to avoid an unnecessary redirect from callback to login)
        ...
        ctx.$auth.redirect('login', true, to);
    }
});
